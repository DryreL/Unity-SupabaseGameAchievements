...
| `Runtime/Unity/` | `DryreLHub.SupabaseGameAchievements.Unity` (precompiled ref to `Newtonsoft.Json.dll`, for `AchievementOverlaySettings`) | `UnityAchievementManager`, `AchievementManagerProxy`, overlay, `UnityWebRequestTransport`, `RemoteConfigAchievementCatalogSource`, icon provider. |
| `Runtime/TextMeshPro/` | `...Unity.TextMeshPro` | `AchievementToastViewTMP`. `versionDefines`-gated on `com.unity.textmeshpro` OR `com.unity.ugui` >= 2.0 (TMP moved into ugui in Unity 6), define `ACHIEVEMENTS_TEXTMESHPRO`. |
...
| Item | File | Notes |
|---|---|---|
| Overlay tab | `Editor/AchievementDashboardWindow.Overlay.cs`, `AchievementOverlaySupport.cs`, `AchievementToastPrefabTools.cs`; runtime `Runtime/Unity/AchievementOverlaySettings.cs` | The toast's look as data: `Resources/Achievements/overlay.json` (Newtonsoft, string enums, hex colors, LF, only the dashboard writes it, autosaved after 0.4 s, never overwrites a file it could not parse). `UnityAchievementManager.ApplyOverlaySettings` loads it after the overlay is bound and calls `UnityAchievementOverlay.ApplySettings` (drops the built view so the next toast is rebuilt; safe at runtime, the dashboard does it live in Play Mode). Sound/font/prefab are stored as Resources paths (`AchievementResourcePaths.FromAssetPath`); the file being present suppresses the `Config.UnlockSound`/`ToastHoldDuration` overrides in `Create` (`OverlaySettingsApplied`). `AchievementToastView` owns placement: `SetCorner` REPLACES the panel's anchors/pivot (six positions; margins always push inwards; slide direction follows the edge), `SetAnimation` (Slide/Fade/Pop), `SetScale`; custom prefabs go through the same calls, so their Panel must have a fixed size. `RoundedBoxSprite` caches by size/radius/colors (max 8). The IMGUI preview re-draws the built-in layout (440x108 panel, icon at 20, texts at x=108) with `GUI.DrawTexture`; keep it in step with `CreateDefault`. The tutorial page's checker (`AchievementToastPrefabTools.Check/ApplyFix/AutoWire`) reads the view's private fields by name through `SerializedObject` (`_panel`, `_canvasGroup`, `_icon`, `_header`, `_title`, `_description`, TMP: `_headerTmp`...): renaming those fields breaks it. Pure parts (settings, resource paths, name-to-role matching, file writing) are tested in `Tests/Editor/OverlaySettingsTests.cs`; the IMGUI, the prefab tools and the PlayMode placement/gap tests were compile-checked only. |
| Achievement Dashboard | `Editor/AchievementDashboardWindow.cs` (UI + PostgREST) and `Editor/AchievementDashboardModel.cs` (data, validation, payloads, merge, manifest) | Editor for every `achievements` column. Working copy autosaved to `Assets/Resources/Achievements/dashboard.json` (moved there from `ProjectSettings/DryreLHub.AchievementDashboard.json` by `DashboardData.MigrateFile`, which keeps the old file if the move fails; it ships in builds like the rest of Resources); each entry stores its server `id` after the first push, so later edits PATCH that row (mutable columns only - the identity trigger rejects `key`/`bit_index` changes). "Modified" is derived by comparing the current fields with `SyncedSnapshot`, not a dirty flag. Service key is session-only like the other windows; `sb_secret_` keys go in `apikey` only, JWT keys also in `Authorization` (same rule as the exporter). New entries get `icon_path = <IconFolder>/<key>` (`IconFolder` defaults to `images`, relative to the runtime icon prefix `Achievements/`); that auto path follows key/folder edits only for never-pushed entries. Localize button: `DashboardData.BuildLocalizationPlan()` (overrides, else `<key>_title`/`<key>_description` in `DefaultLocalizationTable`) is handed to `AchievementLocalizationBridge.Sync`, a hook in the Editor assembly that `Editor/Localization/` (own asmdef, `versionDefines`-gated on `com.unity.localization`, compiled against 1.5.13 only) assigns from an `[InitializeOnLoad]` constructor - the Editor assembly itself never references Unity Localization (same rule as §5). It only adds missing entries, never overwrites text, and writes the text into the English table only (`LocalizationLocales.PickSource`: `en`, else a regional `en-*`, else the first locale); other locales get the shared key, no text. Locales are de-duplicated first (`LocalizationLocales.DistinctIndexes`): the Kiva project lists every locale twice in its Addressables group, and `CreateStringTableCollection` then made an orphan `_en 1` asset per duplicate. The Game setup scan (`AchievementRuntimeSetup.Combine`) also reads the OPEN scenes, so a bootstrap that was added but not saved yet counts (and shows a save reminder); the disk scan alone missed it. The report also records WHICH scenes hold a bootstrap (`Report.BootstrapScenes`) and whether code/a prefab starts it everywhere (`StartedElsewhere`); `SceneLacksStarter` warns when the open scene has none, because pressing Play in such a scene leaves achievements off (the Kiva project had the bootstrap only in MainMenu and its Game scene logged `Report(...) ignored`). `AchievementEvents.WhyNothingListens` says which case it is (system not started vs. no rules loaded). Error banners use `DrawErrorBox` (plain selectable text; a HelpBox showed nothing after the first colon of a long exception) and failures are also written to the Console with the full exception. `DashboardData.Save` tries swap, then copy, then an in-place write with FileShare.ReadWrite|Delete before giving up. Never autosaves over a data file it failed to load. All generated JSON (dashboard file, manifest, rules.json) is written with LF: Newtonsoft indents with `Environment.NewLine`, and comparing CRLF output against a normalized file made the Rules tab report `rules.json` as out of date forever on Windows. A failed autosave is retried every second and only shown after 6 s (a briefly locked file must not flash SAVE FAILED). Logic is tested in `Tests/Editor/AchievementDashboardModelTests.cs`; the IMGUI/network layer was compile-checked only, not driven in a live Editor. |
